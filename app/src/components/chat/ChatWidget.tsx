...
const GREETING: ChatMessage = {
  role: 'model',
  content: "Hi! I'm Sofia, your Steadfast enrollment advisor 👋 I'm here 24/7 to answer your questions about studying in the U.S. What's on your mind?",
  timestamp: Date.now(),
}
...
export default function ChatWidget() {
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'model', content: "Sorry, I'm having trouble connecting. Please try again or email us directly at advisors@steadfaststudentservices.com", timestamp: Date.now() }])
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <>
      ...
      {/* Chat window */}
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col chat-enter overflow-hidden" style={{ maxHeight: '520px' }}>
          {/* Header */}
          <div className="bg-navy px-4 py-3 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-white/50 text-xs">Online 24/7</span>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50" style={{ minHeight: 0 }}>
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={msg.role === 'model' ? 'chat-bubble-bot' : 'chat-bubble-user'}>{msg.content}</div>
              </div>
            ))}
            {loading && (
